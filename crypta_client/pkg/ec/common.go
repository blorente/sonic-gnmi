// Package ec contains helpers and definitions for working with Haven EC host commands.
package ec
...
// CommandDispatcher encapsulates how to send a raw (EC) command to Haven and read the response.
type CommandDispatcher interface {
	// DispatchCommand submits a fully formatted command to the Haven and returns the fully response
	...
	// DispatchCommandNoResponse submits a fully formatted command to the Haven, but discards the
	...
	// Close closes the connection to the Haven, releasing any held resources.
	Close() error
}

// getEcChecksum calculates the Haven EC checksum of the given data.
// The checksum is designed such that the sum of all octets in a valid Haven EC request or response
// is 0 (mod 256).
...
// SendCommand delivers EC command and extra params(if any) to haven using the given command dispatcher.
...
// SendECCommandNoResponse delivers EC command and extra params(if any) to haven using the given command dispatcher but does not parse the response.
...
// HostHeaderLen is the packed size of a Haven EC host request or response header.
const HostHeaderLen = 8

// ecHostRequestHeader is an ec_host_request and has size HostHeaderLen.
// See https://chromium.googlesource.com/chromiumos/platform/ec/+/master/include/ec_commands.h.
...
// formatCommand formats a Haven host command with given parameters as a raw EC command byte array.
...
// PeekResponse is exported for the benefit of Haven transport protocols that rely on the EC
...
func PeekResponse(response []byte) (uint16, error) {
	if len(response) < HostHeaderLen {
		return 0, fmt.Errorf("response (%d bytes) was too short to be a Haven EC response", len(response))
	}
...
// parseResponse parses the complete EC response from the Haven, validates it (including the
...
func parseResponse(response []byte) ([]byte, error) {
	if len(response) > 0xffff+HostHeaderLen { // data length is a uint16, plus size of ecHostResponseHeader
		return nil, fmt.Errorf("response (%d bytes) was too long to be a Haven EC response", len(response))
	}
