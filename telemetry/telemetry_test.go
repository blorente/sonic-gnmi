...
func TestFlags(t *testing.T) {
	...
	tests := []struct {
		args                       []string
		expectedPort               int
		expectedStreamingThreshold int
		expectedUnaryThreshold     int
		expectedIdleDur            int
		expectedLogLevel           int
	}{
		{
			[]string{"cmd", "-port", "9090", "-streaming-threshold", "200", "-unary-threshold", "300", "-idle_conn_duration", "10", "-v", "6", "-noTLS"},
			9090,
			200,
			300,
			10,
		...
		{
			[]string{"cmd", "-port", "2020", "-streaming-threshold", "500", "-unary-threshold", "600", "-idle_conn_duration", "4", "-v", "0", "-insecure"},
			2020,
			500,
			600,
			4,
		...
		{
			[]string{"cmd", "-port", "5050", "-streaming-threshold", "10", "-unary-threshold", "20", "-idle_conn_duration", "3", "-v", "-3", "-noTLS"},
			5050,
			10,
			20,
			3,
		...
		{
			[]string{"cmd", "-port", "8081", "-streaming-threshold", "1", "-unary-threshold", "2", "-idle_conn_duration", "1"},
			8081,
			1,
			2,
			1,
		...
		{
			[]string{"cmd", "-port", "8081", "-streaming-threshold", "1", "-unary-threshold", "1", "-idle_conn_duration", "1", "-server_crt", "../testdata/certs/testserver.cert"},
			8081,
			1,
			1,
			1,
	...
	for index, test := range tests {
		...
		if *config.StreamingThreshold != test.expectedStreamingThreshold {
			t.Errorf("Expected threshold to be %d, got %d", test.expectedStreamingThreshold, *config.StreamingThreshold)
		}

		if *config.UnaryThreshold != test.expectedUnaryThreshold {
			t.Errorf("Expected threshold to be %d, got %d", test.expectedUnaryThreshold, *config.UnaryThreshold)
		}
