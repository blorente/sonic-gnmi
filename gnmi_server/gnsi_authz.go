...
func NewGNSIAuthzServer(srv *Server) *GNSIAuthzServer {
	...
	log.V(lvl.INFO).Infof("gnsi: loading authz metadata from %s", srv.config.AuthzMetaFile)
	log.V(lvl.INFO).Infof("gnsi: loading authz policy from %s", srv.config.AuthzPolicyFile)
...
func (srv *GNSIAuthzServer) Rotate(stream authz.Authz_RotateServer) error {
	...
	if srv.Server.WarmRestartHelper.FetchFreezeStatus() {
		log.V(lvl.ERROR).Info("gNSI authz Rotate RPC disabled since NSF is ongoing!")
		return status.Errorf(codes.Unavailable, "gNSI authz Rotate RPC disabled since NSF is ongoing!")
	}
	ctx := stream.Context()
	...
	if !authzMu.TryLock() {
		log.V(lvl.ERROR).Infof("[%v]gNSI: authz.Rotate already in use", session)
		return status.Errorf(codes.Aborted, "concurrent authz.Rotate RPCs are not allowed")
	}
	defer authzMu.Unlock()

	log.V(lvl.INFO).Infof("[%v]gNSI: Begin authz.Rotate", session)
	defer log.V(lvl.INFO).Infof("[%v]gNSI: End authz.Rotate", session)

	srv.checkpointAuthzFreshness()
	if err := srv.checkpointAuthzFile(); err != nil {
		log.V(lvl.ERROR).Infof("Failure during Authz checkpoint: %v", err)
	}
	for {
		req, err := stream.Recv()
		if err == io.EOF {
			...
			if err := copyFile(srv.config.AuthzPolicyFile+backupExt, srv.config.AuthzPolicyFile); err != nil {
				log.V(lvl.ERROR).Infof("[%v]gnsi: failed to revert authz policy file (%v): %v", session, srv.config.AuthzPolicyFile, err)
			}
			srv.revertAuthzFileFreshness()
			return status.Errorf(codes.Aborted, "No Finalize message")
		}
		if err != nil {
			log.V(lvl.ERROR).Infof("[%v]gnsi: while processing a rotate request got error: `%v`. Reverting to last good state.", session, err)
			// Connection closed without Finalize message. Revert all changes made until now.
			if err := copyFile(srv.config.AuthzPolicyFile+backupExt, srv.config.AuthzPolicyFile); err != nil {
				log.V(lvl.ERROR).Infof("[%v]gnsi: failed to revert authz policy file (%v): %v", session, srv.config.AuthzPolicyFile, err)
			}
...
func (srv *GNSIAuthzServer) processRotateRequest(req *authz.RotateAuthzRequest) (*authz.RotateAuthzResponse, error) {
	...
	log.V(lvl.INFO).Infof("received a gNSI.Authz UploadRequest request message")
	// log.V(lvl.DEBUG).Infof("request message: %v", policyReq)
...
func (srv *GNSIAuthzServer) checkpointAuthzFile() error {
	log.V(lvl.INFO).Infof("Checkpoint authz file: %v", srv.config.AuthzPolicyFile)
...
func (srv *GNSIAuthzServer) checkpointAuthzFreshness() {
	log.V(lvl.INFO).Infof("checkpoint authz freshness")
	srv.authzMetadataCopy = *srv.authzMetadata
}

func (srv *GNSIAuthzServer) revertAuthzFileFreshness() {
	log.V(lvl.INFO).Infof("revert authz freshness")
...
func (srv *GNSIAuthzServer) saveAuthzFileFreshess(path string) error {
	log.V(lvl.INFO).Infof("save authz metadata to file: %v", path)
...
func (srv *GNSIAuthzServer) loadAuthzFreshness(path string) error {
	log.V(lvl.INFO).Infof("load authz metadata from file: %v", path)
