...
func TestTranslSubscribe(t *testing.T) {
	...
	t.Run("SAMPLE_no_interval", func(t *testing.T) {
		...
		sub.VerifyT(
			(sdc.DefaultSampleInterval+2)*time.Second,
			Updated(acl2Path+"/state/description", "foo"),
		)
	})
	...
	t.Run("TARGETDEFINED", func(t *testing.T) {
		...
		updateDb(t, DbDataMap{
			...
			"APPL_STATE_DB": {"PORT_TABLE:Ethernet0": {"description": "the one"}},
		})
	...
	t.Run("bundle_version_0.0.0", func(t *testing.T) {
		...
		sub.Verify(
			Updated("/openconfig-interfaces:interfaces/interface[name=Ethernet0]/config/mtu", uint64(9122)),
			Updated("/openconfig-interfaces:interfaces/interface[name=Ethernet0]/state/mtu", uint64(9122)),
			client.Sync{},
		)
	})
...
func updateDb(t *testing.T, data DbDataMap) {
	...
	for dbName, tableData := range data {
		...
		defer db.CloseRedisClient(redis)
...
func TestDebugSubscribePreferences(t *testing.T) {
	...
	ifIpv6 := &spb_gnoi.SubscribePreference{
		Path:              strToPath("/openconfig-interfaces:interfaces/interface[name=*]/subinterfaces/subinterface[index=*]/openconfig-if-ip:ipv6/addresses"),
		OnChangeSupported: false,
		TargetDefinedMode: SAMPLE,
		WildcardSupported: true,
	}*/
