...
func NewDbJournal(database string) (*DbJournal, error) {
	...
	log.V(lvl.INFO).Infof("Successfully started the DbJournal for %v", journal.database)
	return journal, nil
}
...
func (dbj *DbJournal) cleanup() {
	...
	log.V(lvl.INFO).Infof("DbJournal closed successfully!")
}
...
func (dbj *DbJournal) journal() {
	...
	for {
		...
		case msg := <-dbj.notifications:
			...
			if dErr != nil {
				log.V(lvl.ERROR).Infof("Shutting down %v Journal: %v", dbj.database, dErr)
				return
			}
			event = []string{}
			...
			if err := dbj.rotateFile(); err != nil {
				log.V(lvl.ERROR).Infof("Shutting down DbJournal, failed to manage file rotation: %v", err)
				return
			}
			_, writeErr := dbj.file.Write([]byte(entry + "\n"))
			if writeErr != nil {
				log.V(lvl.ERROR).Infof("Failed to write to DbJournal file: %v", writeErr)
			}
		case <-dbj.done:
			return
		}
