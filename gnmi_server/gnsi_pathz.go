...
func NewGNSIPathzServer(srv *Server) *GNSIPathzServer {
	ret := &GNSIPathzServer{
		...
		pathzPolicy:       srv.config.PathzPolicyFile,
		pathzPolicyBackup: srv.config.PathzPolicyFile + ".backup",
	}
	...
	if srv.config.PathzPolicy {
		if err := ret.pathzProcessor.UpdatePolicyFromFile(ret.pathzPolicy); err != nil {
			log.V(lvl.ERROR).Infof("Failed to load gNMI pathz file %s: %v", ret.pathzPolicy, err)
		}
	}
	return ret
}
...
func (srv *GNSIPathzServer) savePathzPolicyToFile(p *pathz.AuthorizationPolicy) (string, error) {
	...
	log.V(lvl.DEBUG).Infof("Saving pathz policy to file: %s", srv.pathzPolicy)
	return content, attemptWrite(srv.pathzPolicy, []byte(content), 0o644)
}

func (srv *GNSIPathzServer) verifyPathzFile(c string) error {
	content, err := os.ReadFile(srv.pathzPolicy)
	if err != nil {
		return err
	}
	if c != string(content) {
		return fmt.Errorf("Pathz file %s contains error.", srv.pathzPolicy)
	}
	return nil
}
...
func (srv *GNSIPathzServer) createCheckpoint() error {
	...
	return copyFile(srv.pathzPolicy, srv.pathzPolicyBackup)
}
...
func (srv *GNSIPathzServer) revertPolicy() error {
	...
	if srv.policyUpdated {
		...
		if err := srv.pathzProcessor.UpdatePolicyFromProto(srv.policyCopy); err != nil {
			...
			os.Remove(srv.pathzPolicyBackup)
			return err
		}
	}
	srv.pathzMetadata = srv.pathzMetadataCopy
	return os.Rename(srv.pathzPolicyBackup, srv.pathzPolicy)
}
...
func (srv *GNSIPathzServer) Rotate(stream pathz.Pathz_RotateServer) error {
	...
	if err := fileCheck(srv.pathzPolicy); err != nil {
		log.V(lvl.ERROR).Infof("Error in reading file %s: %v", srv.pathzPolicy, err)
		return status.Errorf(codes.NotFound, "Error in reading file %s: %v", srv.pathzPolicy, err)
	}
