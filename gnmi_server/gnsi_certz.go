...
func NewGNSICertzServer(srv *Server) *GNSICertzServer {
	log.V(lvl.INFO).Info("Starting gNSI Certz Server")
	s := &GNSICertzServer{
		Server:   srv,
	...
	log.V(lvl.INFO).Infof("Integrity Manifest File: %v", integrityManifestFile)
	legacyCrlPath = srv.config.CertCRLConfig
	...
	if _, err := os.Stat(crlPath); err != nil && os.IsNotExist(err) {
		log.V(lvl.INFO).Infof("Creating CRL Default path: %v", crlPath)
		if err := os.MkdirAll(crlPath, 0777); err != nil {
			log.V(lvl.WARNING).Infof("Failed Creating CRL Default dir: %v", crlPath)
		}
	}
	flushPath := filepath.Join(srv.config.CertCRLConfig, crlDefault+crlFlush)
	if _, err := os.Stat(flushPath); err != nil && os.IsNotExist(err) {
		log.V(lvl.INFO).Infof("Creating CRL Flush path: %v", flushPath)
		if err := os.MkdirAll(flushPath, 0777); err != nil {
			log.V(lvl.WARNING).Infof("Failed Creating CRL Flush dir: %v", flushPath)
		}
	}
	log.V(lvl.INFO).Info("Certz Server Ready")
	return s
}
...
func (srv *GNSICertzServer) GetIntegrityManifest(context.Context, *certz.GetIntegrityManifestRequest) (*certz.GetIntegrityManifestResponse, error) {
	...
	if err != nil {
		...
		return nil, status.Errorf(codes.Aborted, "Failed to read manifest %s: %v", integrityManifestFile, err)
	}
	return &certz.GetIntegrityManifestResponse{Manifest: content}, nil
}

func (srv *GNSICertzServer) bootstrapDefaultProfile() *profile {
	log.V(lvl.INFO).Info("Bootstrapping default profile")
...
func (srv *GNSICertzServer) Rotate(stream certz.Certz_RotateServer) error {
	...
	if srv.Server.WarmRestartHelper.FetchFreezeStatus() {
		log.V(lvl.ERROR).Info("gNSI cert Rotate RPC disabled since NSF is ongoing!")
		return status.Errorf(codes.Unavailable, "gNSI cert Rotate RPC disabled since NSF is ongoing!")
	}
	ctx := stream.Context()
	...
	if !certzMu.TryLock() {
		log.V(lvl.ERROR).Infof("[%v]gNSI: certz.Rotate already in use", session)
		return status.Error(codes.Aborted, "concurrent certz.Rotate RPCs are not allowed")
	}
	defer certzMu.Unlock()

	log.V(lvl.INFO).Infof("[%v]gNSI: Begin certz.Rotate", session)
	defer log.V(lvl.INFO).Infof("[%v]gNSI: End certz.Rotate", session)

	var profileId string
	for {
		req, err := stream.Recv()
		if err == io.EOF {
			log.V(lvl.ERROR).Infof("[%v]gNSI: Received unexpected EOF", session)
		...
		if err != nil {
			log.V(lvl.ERROR).Infof("[%v]gNSI: Received error: %v", session, err)
			// Connection closed without Finalize message. Revert all changes made until now.
			srv.revertProfile(profileId)
		...
		if endReq := req.GetFinalizeRotation(); endReq != nil {
			...
			log.V(lvl.INFO).Infof("[%v]gNSI: Received Finalize: %v", session, endReq)
			// The new credentials have been confirmed to be correct. Remove the old ones.
			if err := srv.finalizeProfile(profileId); err != nil {
				log.V(lvl.ERROR).Infof("[%v]gNSI: Failed to remove the old credentials: %v", session, err)
				return status.Errorf(codes.Unknown, "Failed to remove the old credentials: %v", err)
			}
			log.V(lvl.INFO).Infof("[%v]gNSI: Rotation Finalized", session)
			return nil
		}
		if profileId == "" {
		...
		log.V(lvl.INFO).Infof("[%v]gNSI: Rotating profile: %s", session, profileId)
		resp, err := srv.processRotateRequest(profileId, req)
		if err != nil {
			log.V(lvl.ERROR).Infof("[%v]gNSI: Processing of rotate request resulted in an error: %v", session, err)
			srv.revertProfile(profileId)
			return err
		}
		// Send confirmation that the UploadRequest was processed.
		if err := stream.Send(resp); err != nil {
			log.V(lvl.ERROR).Infof("Got an error while sending UploadResponse: %v", err)
			srv.revertProfile(profileId)
...
func (srv *GNSICertzServer) newGenericEntity(eType CertzType, profileID string, entityMsg *certz.Entity) *genericEntity {
	...
	log.V(lvl.INFO).Infof("creating new Entity: %v", eType.String())
...
func (srv *GNSICertzServer) activateEntity(profileID string, entity *genericEntity) error {
	...
	log.V(lvl.INFO).Infof("Activating: %s", entity.EType.String())
	switch entity.EType {
	case certType:
...
func (srv *GNSICertzServer) revertProfile(profileID string) {
	log.V(lvl.INFO).Infof("Reverting to last known good gRPC credentials for profile=%v", profileID)
	if profileID == "" {
		profileID = defaultProfile
		log.V(lvl.INFO).Infof("Reverting default profile: %v", defaultProfile)
	}
	profile, ok := srv.profiles[profileID]
	if !ok || profile == nil {
		log.V(lvl.INFO).Infof("No profile to revert: %v", profileID)
		return
	}
	if profile.ActiveEntities.Cert.Final == false {
		log.V(lvl.INFO).Info("Rollback Cert")
		if err := atomicSetSrvCertKeyPair(srv.config, profile.LastEntities.Cert.CertPath, profile.LastEntities.Cert.KeyPath); err != nil {
			log.V(lvl.ERROR).Infof("Failed to revert certificate files: %e", err)
		}
	...
	if profile.ActiveEntities.TrustBundle.Final == false {
		log.V(lvl.INFO).Info("Rollback TB")
		if err := atomicSetCACert(srv.config, profile.LastEntities.TrustBundle.CertPath); err != nil {
			log.V(lvl.ERROR).Infof("Failed to revert trust bundle file: %e", err)
		}
	...
	if profile.ActiveEntities.CrlBundle.Final == false {
		log.V(lvl.INFO).Info("Rollback CRL")
		// translate from v0 to v1 defaults
		crlTranslate := profileID
		if crlTranslate == defaultProfile {
			crlTranslate = crlDefault
...
func (srv *GNSICertzServer) finalizeProfile(profileID string) error {
	log.V(lvl.INFO).Infof("Finalizing gRPC credentials for profile=%v", profileID)
	profile, ok := srv.profiles[profileID]
	if !ok || profile == nil {
	...
	if profile.ActiveEntities.Cert.Final != true {
		log.V(lvl.INFO).Info("Finalize Cert")
	...
	if profile.ActiveEntities.TrustBundle.Final != true {
		log.V(lvl.INFO).Info("Finalize TB")
		removeEntityFiles(profile.LastEntities.TrustBundle)
		profile.ActiveEntities.TrustBundle.Final = true
	}
	profile.LastEntities.TrustBundle = profile.ActiveEntities.TrustBundle

	if profile.ActiveEntities.CrlBundle.Final != true {
		log.V(lvl.INFO).Info("Finalize CRL")
		// translate from v0 to v1 defaults
		crlTranslate := profileID
		if crlTranslate == defaultProfile {
			crlTranslate = crlDefault
...
func writeEntityFreshness(profileID string, entity *genericEntity) {
	log.V(lvl.INFO).Infof("Writing %s profile to DB: %s", profileID, entity.EType.String())
	var vFld string
	var cFld string
	...
	default:
		log.V(lvl.ERROR).Infof("Unknown entity in profile: %s : %s", profileID, entity.EType.String())
	}
...
func saveCertzMetadata(path string, profiles map[string]*profile) error {
	log.V(lvl.INFO).Info("Saving gRPC credentials metadata.")
	data, err := json.Marshal(profiles)
	if err != nil {
		return err
	}
	log.V(lvl.INFO).Infof("Certz Metadata Path: %v", path)
	return attemptWrite(path, data, 0644)
}

func loadCertzMetadata(path string, profiles map[string]*profile) error {
	log.V(lvl.INFO).Infof("Loading metadata from `%v`...\n", path)
	...
	for k, p := range profiles {
		if err := validateProfileExists(*p); err != nil {
			log.V(lvl.WARNING).Infof("%v profile error: %v", k, err)
			delete(profiles, k)
		}
		log.V(lvl.INFO).Infof("Added %s profile: %+v", k, *p)
	}
	return nil
}
...
func rotateCRLBundle(crl *certz.CertificateRevocationListBundle, path string) error {
	log.V(lvl.INFO).Infof("Saving CRL to: %s", path)
	...
	for _, file := range dstFiles {
		if err := os.Remove(filepath.Join(path, file.Name())); err != nil {
			log.V(lvl.WARNING).Infof("Failed to remove: %s: %v;", file.Name(), err)
		}
	}
...
func copyCRLBundle(srcPath string, dstPath string) {
	log.V(lvl.INFO).Infof("Copying CRL Bundle from: %s to: %s", srcPath, dstPath)
	...
	for _, file := range srcFiles {
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Failed to read: %s : %v", file.Name(), err)
			continue
		}
...
func rmSymlink(path string) (string, error) {
	...
	log.V(lvl.INFO).Infof("Removing sym link: %s", path)
...
func restoreSymlink(oldTarget, link string) error {
	// NOTE: muPath has to be writer-locked when entering this function.
	log.V(lvl.INFO).Infof("Restoring sym link: %s  old: %s", link, oldTarget)
	if oldTarget == "" {
		return nil
	}
...
func rmFileIfNotPointedToBySymlink(file, link string) error {
	...
	log.V(lvl.INFO).Infof("Removing file: %s link: %s", file, link)
...
func isSymlinkValid(path string) bool {
	// NOTE: muPath has to be writer-locked when entering this function.
	log.V(lvl.INFO).Infof("Validating sym link: %s", path)
...
func atomicSetSrvCertKeyPair(cfg *Config, sCert, sKey string) error {
	...
	log.V(lvl.INFO).Infof("Attempting to set Cert: %s", sCert)
	...
	log.V(lvl.INFO).Infof("Succesful Set Cert: %s", sCert)
	return nil
}
...
func atomicSetCACert(cfg *Config, caCert string) error {
	// NOTE: muPath has to be writer-locked when entering this function.
	// This assumes default profile and needs to be replaced when multi-profile is supported
	log.V(lvl.INFO).Infof("Attempt Set CA: %s", caCert)
	...
	log.V(lvl.INFO).Infof("Succesful Set CA: %s", caCert)
	return nil
}

func restoreFromFile(link string, file string) string {
	log.V(lvl.INFO).Infof("Restoring: %s", file)
	if _, err := os.Lstat(link); !os.IsNotExist(err) {
	...
	if _, err := os.Stat(file); os.IsNotExist(err) {
		log.V(lvl.ERROR).Infof("cannot find file: %s link: %s", file, link)
		return "unknown"
	}
	return file
}
...
func removeEntityFiles(e *genericEntity) {
	switch e.EType {
	case certType:
		log.V(lvl.INFO).Infof("Removing Cert: %s", e.CertPath)
		if err := os.Remove(e.CertPath); err != nil {
			log.V(lvl.WARNING).Infof("Removing Cert failed: %s", err)
		}
		log.V(lvl.INFO).Infof("Removing Key: %s", e.KeyPath)
		if err := os.Remove(e.KeyPath); err != nil {
			log.V(lvl.WARNING).Infof("Removing Key failed: %s", err)
		}
	case tbType:
		log.V(lvl.INFO).Infof("Removing TrustBundle: %s", e.CertPath)
		if err := os.Remove(e.CertPath); err != nil {
			log.V(lvl.WARNING).Infof("Removing TrustBundle failed: %s", err)
		}
	}
}
