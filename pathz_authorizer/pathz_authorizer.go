...
func (result *Result) updateResult(p permission, mode pathzpb.Mode, rule string) error {
	...
	case pathzpb.Mode_MODE_READ:
		access = p.read
		// TODO(b/265372119): Legacy Pathz. TO be removed.
		if p.write.action == pathzpb.Action_ACTION_PERMIT {
			access = p.write
		}
		// END legacy
...
func (result *Result) logResult(user string, path *gnmipb.Path, mode pathzpb.Mode) {
	...
	if result.Action == pathzpb.Action_ACTION_UNSPECIFIED {
		log.V(lvl.INFO).Infof("User %s with %s request on %s does not match any gNMI ACL rule. Request denied.", user, modeStr, printPath(path.GetElem()))
	} else if result.Action == pathzpb.Action_ACTION_DENY {
		log.V(lvl.INFO).Infof("User %s with %s request on %s matched gNMI ACL rule %s (rule ID: %s). Request denied.", user, modeStr, printPath(path.GetElem()), result.MatchedRule, result.RuleId)
	}
}
...
func (node *gnmiAuthzNode) authorize(user string, path []*gnmipb.PathElem, mode pathzpb.Mode, nameIdx int, keys []string, keyIdx int, groups map[string]stringSet) Result {
	...
	if len(keys) > 0 {
		...
		if keyIdx >= len(keys) {
			log.V(lvl.ERROR).Infof("Key index got out of range for key list %v", keys)
			return ret
		}

		// Key value mismatches.
		if keys[keyIdx] != node.key {
			log.V(lvl.ERROR).Infof("Key value mismatches. Expect %v, got %v", node.key, keys[keyIdx])
			return ret
		}
		...
		if !ok {
			log.V(lvl.ERROR).Infof("Key name %v does not exist in path", node.key)
			return ret
		}
...
func (processor *GnmiAuthzProcessor) Authorize(user string, path *gnmipb.Path, mode pathzpb.Mode) (*Result, error) {
	if processor == nil {
		log.V(lvl.ERROR).Info("Authorize error: nil pointer")
		return nil, fmt.Errorf("processor cannot be nil")
	}
	if mode == pathzpb.Mode_MODE_UNSPECIFIED {
		log.V(lvl.ERROR).Infof("Authorize error: undefined access mode")
...
func (processor *GnmiAuthzProcessor) UpdatePolicyFromFile(policyFile string) error {
	if processor == nil {
		log.V(lvl.ERROR).Info("UpdatePolicyFromFile error: nil pointer")
		return fmt.Errorf("processor cannot be nil")
	}

	content, err := os.ReadFile(policyFile)
	if err != nil {
		log.V(lvl.ERROR).Infof("Failed to open file %s.", policyFile)
		return err
	}
	policy := &pathzpb.AuthorizationPolicy{}
	err = proto.UnmarshalText(string(content), policy)
	if err != nil {
		log.V(lvl.ERROR).Infof("Failed to parse file %s.", policyFile)
		return err
	}
...
func (processor *GnmiAuthzProcessor) UpdatePolicyFromProto(policyProto *pathzpb.AuthorizationPolicy) error {
	if processor == nil {
		log.V(lvl.ERROR).Info("UpdatePolicyFromProto error: nil pointer")
		return fmt.Errorf("processor cannot be nil")
	}

	newRoot, newGroups, err := createNewPolicy(policyProto)
	if err != nil {
		log.V(lvl.ERROR).Infof("Failed to create new gNMI authorization rule.")
		return err
	}
	processor.mux.Lock()
	defer processor.mux.Unlock()
