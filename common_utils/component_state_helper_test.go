...
func clearComponentState(t *testing.T) {
	...
	if keys, err := dbc.Keys(context.Background(), componentStateTable+sep+"*").Result(); err == nil {
		for _, key := range keys {
			if err = dbc.Del(context.Background(), key).Err(); err != nil {
				t.Fatalf("Failed to clear component state information in DB: %v\n", err)
			}
		}
	}
	if err = dbc.Del(context.Background(), alarmStatusTable).Err(); err != nil {
...
func readAlarmStatus(t *testing.T) bool {
	...
	defer db.CloseRedisClient(dbc)
	value, err := dbc.HGet(context.Background(), alarmStatusTable, alarmStatusKey).Result()
...
func TestContainerMonitorSetsInactiveForNonessentialComponent(t *testing.T) {
	...
	hash["state"] = "INACTIVE"
	sep, _ := sdcfg.GetDbSeparator(dbName, "")
	if err := dbc.HMSet(context.Background(), componentStateTable+sep+"component", hash).Err(); err != nil {
		t.Fatalf("Failed to write to Redis DB :%v", err)
	}
