...
func (component SystemComponent) String() string {
	...
	case SwssCfgmgr:
		return "swss:cfgmgr"
	default:
		log.V(lvl.ERROR).Infof("Invalid SystemComponent")
...
func getRedisDBClient() (*redis.Client, error) {
	...
	rclient := db.TransactionalRedisClient(db.DBNum(id))
	if rclient == nil {
...
func (h *ComponentStateHelper) callLuaScript(state ComponentState, reason string, hwErr bool) error {
	...
	if updated != "true" {
		log.V(lvl.ERROR).Infof("Component %s failed to updated state from %s to %v with reason: %s.", h.componentId, oldState, state, reason)
		return fmt.Errorf("Component %s failed to update state from %s to %v with reason: %s.", h.componentId, oldState, state, reason)
	}
	log.V(lvl.INFO).Infof("Component %s successfully updated state from %s to %s with reason: %s.", h.componentId, oldState, newState, reason)
...
func NewSystemStateHelper() (*SystemStateHelper, error) {
	...
	if keys, err := h.db.Keys(context.Background(), componentStateTable+sep+"*").Result(); err == nil {
		for _, key := range keys {
			...
			if len(splitedTableKeys) < 2 {
				log.V(lvl.ERROR).Infof("Missing table key in %s.", key)
				continue
			}
	...
	if err := h.writeSystemCritical(h.IsSystemCritical()); err != nil {
		log.V(lvl.ERROR).Infof("Failed writing state to DB: %s", err.Error())
	}
