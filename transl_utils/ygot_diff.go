...
// DiffResult holds updated {path, value} pairs and deleted paths
...
// DiffResults holds a map of timestamp to DiffResult
type DiffResults map[int64]*DiffResult
...
func Diff(original, modified ygot.GoStruct, ygsMeta ygot.YGStructMetaMap, opts DiffOptions) (DiffResults, error) {
	var yd ygotDiff
	yd.DiffOptions = opts
	yd.errors.Separator = "\n"
	yd.ygsMeta = ygsMeta
	yd.DiffResults = make(DiffResults)
	...
	for i := v1.NumField() - 1; i >= 0; i-- {
		...
		case reflect.Ptr: // leaf or container
			if ft.Type.Elem().Kind() == reflect.Struct {
				...
				yd.forLeaf(p2, &ft, unboxPtr(f1), unboxPtr(f2))
			}
		case reflect.Map: // list
			yd.forList(&ft, &f1, &f2)
		case reflect.Slice: // leaf-list
			yd.forLeafList(p2, &ft, &f1, &f2)
		case reflect.Int64: // enum
			yd.forEnum(p2, &ft, f1.Int(), f2.Int())
		case reflect.Interface: // union
			yd.forUnion(p2, &ft, &f1, &f2)
		case reflect.Bool: // empty leaf
			yd.forLeaf(p2, &ft, f1.Bool(), f2.Bool())
		default:
...
func (yd *ygotDiff) getLeafTimestamp(parent interface{}, fv interface{}) int64 {
	if parent == nil || yd.ygsMeta == nil {
		return 0
	}
	p, ok := parent.(ygot.GoStruct)
	if !ok {
		return 0
	}
	m := yd.ygsMeta[p]
	...
	if m.LeafMeta == nil || len(m.LeafMeta) == 0 {
		return m.TimestampNano
	}
	if lm := m.LeafMeta[fv]; lm != nil {
		return lm.TimestampNano
	}
	return m.TimestampNano
}
...
func (yd *ygotDiff) forUnion(parent interface{}, f *reflect.StructField, v1, v2 *reflect.Value) {
	if v2.IsNil() {
		if !v1.IsNil() {
			yd.recordDelete(f)
		}
	...
	if yd.RecordAll || v1.IsNil() {
		yd.recordUpdate(parent, f, v2.Interface())
		return
	}
...
func (yd *ygotDiff) forEnum(parent interface{}, f *reflect.StructField, v1, v2 int64) {
	if v2 == 0 {
		if v1 != 0 {
			yd.recordDelete(f)
		}
	...
	if !ok {
		...
		yd.recordUpdate(parent, f, enumDef.Name)
	}
}

func (yd *ygotDiff) forLeaf(parent interface{}, f *reflect.StructField, v1, v2 interface{}) {
	if v2 == nil {
		if v1 != nil {
			yd.recordDelete(f)
		}
	} else if yd.RecordAll || v1 != v2 {
		yd.recordUpdate(parent, f, v2)
	}
}

func (yd *ygotDiff) forLeafList(parent interface{}, f *reflect.StructField, v1, v2 *reflect.Value) {
	var len1, len2 int
	if !v1.IsNil() {
		len1 = v1.Len()
	}
...
func (yd *ygotDiff) recordUpdates(v interface{}) {
	...
	msg, err := ygot.TogNMINotificationsWithMeta(s, 0,
		ygot.GNMINotificationsConfig{UsePathElem: true}, yd.ygsMeta)
	if err != nil {
	...
	for _, m := range msg {
		...
		ts := m.Timestamp
		if yd.DiffResults[ts] == nil {
			yd.DiffResults[ts] = &DiffResult{}
		}
		yd.DiffResults[ts].Update = append(yd.DiffResults[ts].Update, m.Update...)
	}
}
...
// the value v.
func (yd *ygotDiff) recordUpdate(parent interface{}, f *reflect.StructField, v interface{}) {
	...
	ts := yd.getLeafTimestamp(parent, v)
	if yd.DiffResults[ts] == nil {
		yd.DiffResults[ts] = &DiffResult{}
	}
	yd.DiffResults[ts].Update = append(yd.DiffResults[ts].Update, u)
}
...
func (yd *ygotDiff) recordDeletePath(suffix *gnmi.PathElem) {
	if yd.DiffResults[0] == nil {
		yd.DiffResults[0] = &DiffResult{}
	}
	yd.DiffResults[0].Delete = append(yd.DiffResults[0].Delete, yd.newPath(suffix))
}
