...
func createServer(t *testing.T, cfg *sds.Config) *sds.Server {
	...
	if err != nil {
		t.Fatalf("could not generate server's certificate and key pair: %s", err)
	}
...
func getConfigDbClient(t *testing.T) *redis.Client {
	...
	dbId, err := sdcfg.GetDbId("CONFIG_DB", ns)
	if err != nil {
		t.Fatal("failed to get db ", err)
	}
	rclient := db.TransactionalRedisClientWithOpts(&redis.Options{
...
func prepareDb(t *testing.T) {
	...
	mpi_counter := loadConfig(t, "COUNTERS_oid_0x1000000000039", countersEthernet68Byte)
	...
	mpi_counter = loadConfig(t, "COUNTERS_oid_0x1000000000003", countersEthernet1Byte)
	loadDB(t, rclient, mpi_counter)
	...
	mpi_counter = loadConfig(t, "COUNTERS_oid_0x1500000000092a", counters92aByte)
	loadDB(t, rclient, mpi_counter)
