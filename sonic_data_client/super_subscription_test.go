...
var (
	SAMPLE_SUB = &pb.SubscriptionList{
		...
		Subscription: []*pb.Subscription{
			{
				Path:           &pb.Path{Elem: []*pb.PathElem{{Name: "qos"}}},
	...
	MIXED_SUB = &pb.SubscriptionList{
		...
		Subscription: []*pb.Subscription{
			...
			{
				Path:           &pb.Path{Elem: []*pb.PathElem{{Name: "components"}}},
				Mode:           pb.SubscriptionMode_SAMPLE,
				SampleInterval: 30000000000,
			},
			{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectedOk && superSub == nil {
				...
				t.Errorf("findSuperSubscription incorrectly returned a non-nil superSubscription: want=%v, got=%v")
			}
		})
	}
}
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for interval, tInfos := range test.ticker_map {
				for _, tInfo := range tInfos {
					if tInfo.t == nil {
						t.Errorf("Nil ticker in ticker_map: %v")
					}
