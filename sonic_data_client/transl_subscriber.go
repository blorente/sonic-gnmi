...
func (ts *translSubscriber) notify(v *translib.SubscribeResponse) error {
	...
	for _, msg := range msgs {
		...
		if ts.client.superSub != nil && (v == nil || !v.IsOnChangeUpdate) {
...
func defaultMsgBuilder(v *translib.SubscribeResponse, ts *translSubscriber) ([]*gnmipb.Notification, error) {
	...
	if v.Update != nil {
		var err error
		ntfs, err = ts.ygotToScalarValues(p.GetElem(), extraPrefix, v.Update, v.Timestamp, v.YGSMeta)
		if err != nil {
			return nil, err
		}
	}
...
func (ts *translSubscriber) ygotToScalarValues(prefix []*gnmipb.PathElem, prefixElem *gnmipb.PathElem, obj ygot.ValidatedGoStruct, timestamp int64, metaMap ygot.YGStructMetaMap) ([]*gnmipb.Notification, error) {
	ntfs := []*gnmipb.Notification{}
	tmps, err := ygot.TogNMINotificationsWithMeta(obj, timestamp,
		ygot.GNMINotificationsConfig{
			UsePathElem:    true,
			PathElemPrefix: prefix,
	...
	for _, tmp := range tmps {
		if prefixElem != nil {
			for _, u := range tmp.Update {
				insertFirstPathElem(u.Path, prefixElem)
			}
		}
		ntfs = append(ntfs, tmp)
	}
...
func (c *ygotCache) msgBuilder(v *translib.SubscribeResponse, ts *translSubscriber) ([]*gnmipb.Notification, error) {
	...
	res, err := transl_utils.Diff(old, v.Update, v.YGSMeta,
	...
	var ntfs []*gnmipb.Notification
	prefix := ts.toPrefix(v.Path)
	for t, r := range res {
		if len(r.Update) == 0 && len(r.Delete) == 0 {
			continue
		}
		if t == 0 {
			t = v.Timestamp
		}
		ntf := &gnmipb.Notification{
			Timestamp: t,
			Prefix:    prefix,
			Update:    r.Update,
			Delete:    r.Delete,
