...
func createSuperSubscription(subscription *gnmipb.SubscriptionList) *superSubscription {
	...
	if superSub, ok := superSubs.subs[newSuperSub]; ok {
		// This should never happen.
		log.V(lvl.ERROR).Infof("Super Subscription (%p) for %v already exists but a new has been created!", superSub, subscription)
	}
...
func deleteSuperSubscription(superSub *superSubscription) {
	if superSub == nil {
		log.V(lvl.ERROR).Info("deleteSuperSubscription called on a nil Super Subscription!")
		return
	}
...
func (c *TranslClient) leaveSuperSubscription() {
	...
	if len(c.superSub.clients) == 0 {
		...
		log.V(lvl.INFO).Infof("SuperSubscription (%s) closing!", c.superSub)
		...
		log.V(lvl.INFO).Infof("SuperSubscription (%s): %p is now the primary client", c.superSub, c.superSub.primaryClient)
	}
}
...
func (c *TranslClient) addClientToSuperSubscription(subscription *gnmipb.SubscriptionList) {
	...
	log.V(lvl.INFO).Infof("SuperSubscription (%s): added new client=%p", superSub, c)
}
