...
func (c *TranslClient) Get(w *sync.WaitGroup) ([]*spb.Value, error) {
	...
	for i := 0; i < min(numPaths, maxWorkers); i++ {
		workerWg.Add(1)
...
func processGetWorker(pathChan <-chan pathFromGetReq, valueChan chan<- *spb.Value, errChan chan<- error, wg *sync.WaitGroup) {
	...
	for path := range pathChan {
		log.V(lvl.DEBUG).Infof("getWorker processing path: %v", path)
...
func (c *TranslClient) StreamRun(q *queue.PriorityQueue, stop chan struct{}, w *sync.WaitGroup, subscribe *gnmipb.SubscriptionList) {
	...
	for i := 0; i < min(len(subscribe.Subscription), maxWorkers); i++ {
		wg.Add(1)
	...
	for i, pInfo := range subSupport {
		...
		if subscribe_mode == gnmipb.SubscriptionMode_SAMPLE {
			...
			if interval == 0 {
				interval = DefaultSampleInterval * int(time.Second)
	...
	for !c.isPrimary() {
		select {
		case <-c.wakeChan:
			log.V(lvl.INFO).Infof("Secondary client (%p) waking up: isPrimary=%v, tickers=%v", c, c.isPrimary(), intervalToTickerInfoMap)
			continue
		case <-c.channel:
			log.V(lvl.INFO).Infof("Secondary client (%p) received close signal", c)
			return
		}
	}
...
func buildSelectCases(intervalToTickerInfoMap map[int][]*ticker_info, closeChan <-chan struct{}) ([]reflect.SelectCase, map[int]int) {
	...
	for interval, tickers := range intervalToTickerInfoMap {
		...
		ticker := tickers[0]
		if ticker == nil {
			log.V(lvl.ERROR).Infof("Failed to build select case for interval %v because ticker is nil!", interval)
			continue
		}
